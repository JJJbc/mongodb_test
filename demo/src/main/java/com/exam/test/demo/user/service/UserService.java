package com.exam.test.demo.user.service;
...
public class UserService {
  ...
  public ResponseDto<UserDto> getUserInfo(String Id) throws CustomException{
    ...
    if (userOptional.isPresent()){
      ...
      UserDto userDto = new UserDto(user.getUserId(), null, user.getUserName(), user.getRoles()); // 비밀번호 노출 x
  ...
  public ResponseDto<UserDto> signin(String userId, String pwd, HttpServletResponse response)throws CustomException{
    ...
    if (userOptional.isPresent()){
      ...
      if(passwordEncoder.matches(pwd, user.getPwd())){
        String accessToken = tokenService.generateToken(user.getUserId()); // 엑세스 토큰 생성
