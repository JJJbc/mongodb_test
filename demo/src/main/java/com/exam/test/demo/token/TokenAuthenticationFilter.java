...
public class TokenAuthenticationFilter extends OncePerRequestFilter{
  ...
  protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain filterChain) throws ServletException, IOException{
    String token = req.getHeader("Authorization"); //"Bearer" 접두사 포함 여부 확인
    ...
    if (refreshToken != null && tokenService.validateRefreshToken(refreshToken)){
      ...
      try {
        ...
        if(userId != null && SecurityContextHolder.getContext().getAuthentication() == null && tokenService.validateToken(token, userId)) {
